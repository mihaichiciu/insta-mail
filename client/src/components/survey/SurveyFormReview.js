...
const SurveryFormReview = ({ onCancel, formValues, submitSurvey, history }) => {
  const reviewFields = formFields.map(({ name, label }) => (
    <div key={name} style={{ marginTop: '16px', marginBottom: '4px' }}>
      <label>{label}</label>
      <div style={{ marginBottom: '5px', minHeight: '42px' }}>{formValues[name]}</div>
    </div>
  ));
...
function mapStateToProps(state) {
  return {
    formValues: state.form.surveyForm.values, // this will be passed as props to our component
  };
}
